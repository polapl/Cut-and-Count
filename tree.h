...
class Bag {
 public:
  ...
  Bag(Bag::BagType type, Bag* parent, Bag** ref,
      const std::vector<Node>& nodes);

  Bag(Bag::BagType type, const Node& introduced_node);
  Bag(Bag::BagType type, const std::pair<Node, Node>& introduced_edge,
      int edge_weight);
  ...
  std::vector<Bag*> Generate(const std::vector<Bag::BagType>& types,
                             int probability);
...
class Tree {
 public:
  Tree() {}
  Tree(int tree_width, int max_weight)
      : tree_width(tree_width), max_weight(max_weight) {}
  Bag* root;
  int tree_width;
  int max_weight;
